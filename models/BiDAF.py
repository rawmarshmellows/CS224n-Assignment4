...
def get_optimizer(opt, learning_rate):
    if opt == "adam":
        ...
        optfn = tf.train.AdadeltaOptimizer(learning_rate)
    else:
        assert (False)
    return (optfn)
...
class BiDAF(object):
    ...
    def evaluate_answer(self, session, data, num_samples, use_best_span):
        ...
        # First randomly sample from the dataset
        samples = get_random_samples(data, num_samples)

        # Now we whether finding the best span improves the score
        start_indicies, end_indicies = self.predict_for_batch(session, samples, use_best_span)
        pred_answer, truth_answer = self._get_sentences_from_indicies(samples, start_indicies, end_indicies)
    ...
    def run_epoch(self, session, train, val, log=False):
        ...
        for i, train_batch in enumerate(
            ...
            if i % self.config.eval_num == 0:
                ...
                f1_train, EM_train = self.evaluate_answer(session, train,
                                                          num_samples=self.config.samples_used_for_evaluation,
                                                          use_best_span = False)

                # Then evaluate on the val set
                f1_val, EM_val = self.evaluate_answer(session, val, num_samples=self.config.samples_used_for_evaluation,
                                                      use_best_span = False)
